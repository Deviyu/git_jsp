					...
					<div class="text-center">
						<ul class="pagination">
							...
							%>
								...
								<%} else {%>
									<li><a href = "<%=request.getContextPath() %>/userPagingList?page=<%=pageNum-1 %>&pageSize=<%=pageVO.getPageSize() %>">≪</a></li>
								<%}%>
								...
								<%} else {%>
									<li>
										<a href = "<%=request.getContextPath() %>/userPagingList?page=<%=pageNum+1%>&pageSize=<%=pageVO.getPageSize()%>">≫</a>
								<%} %>
								</li>
						</ul>
					</div>
